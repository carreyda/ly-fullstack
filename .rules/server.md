...
- 首次本地开发通过 `pnpm setup` 初始化 PostgreSQL、创建数据库和种子数据，并为 admin、admin-api 生成 development、test、production 六个运行环境文件；仓库只提交各应用的 `.env.example`，运行文件与根 `.env` 均不得提交。其他服务新增私密配置时分别维护自己的模板和本地文件。
