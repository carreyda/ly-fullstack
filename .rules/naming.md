...
| 类型               | 格式                          | 示例                                 |
| ------------------ | ----------------------------- | ------------------------------------ |
| 文件名             | kebab-case                    | `user-info.ts`、`service-im.ts`      |
...
| Pinia 持久化 key   | `APP_PINIA_<NAME>`            | `APP_PINIA_CHAT`                     |
...
```ts
export const STORAGE_TOKEN_KEY = 'APP_TOKEN';
export const STORAGE_USER_KEY = 'APP_USER';
```
...
```
service-im.ts               # 服务实例
service-im-interceptor.ts   # 对应拦截器
