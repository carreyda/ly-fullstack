...
- 图片滑块的缺口图和拼图块由 Admin API 生成，正确位置不下发给浏览器；挑战校验和登录凭证都只能消费一次。创建图片和校验位置默认各限制为每分钟 20 次。
- Nginx 必须传递 `X-Real-IP` 和 `X-Forwarded-For`。Admin API 只信任本机 loopback 代理，不能把服务端口直接暴露到公网后仍假设转发头可信。
- 多实例部署必须把限流和一次性挑战存储上移到 API 网关、WAF 或 Redis 等共享设施。自建图片滑块不等于专业风控，高风险公网场景建议叠加云厂商成熟的人机验证。
