...
- 后台认证：未登录跳转、错误密码、滑块指针拖动与服务端验证码校验、正确登录、用户信息恢复、刷新会话、退出和路由保护。
...
验证码用例会发送仅在 `APP_ENV=test` 下生效的 Playwright 标记，由 Admin API 返回 `testOffset` 作为目标坐标。Playwright 仍会在页面上执行真实 Pointer 拖动并调用真实校验接口，服务端的一次性挑战、偏移容差和防重放路径都会执行；用例不覆盖人眼判断或图像识别步骤。
