...
| C 端 API | NestJS 11 + Fastify              | `apps/api`，默认端口 3000；当前只提供健康检查            |
| 管理 API | NestJS 11 + Fastify              | `apps/admin-api`，默认端口 3001；管理端独立认证边界      |
...
- **服务端只使用 shared 类型入口**：`api` 与 `admin-api` 只能从 `@repo/shared/types` 导入共享类型，不得依赖 `@repo/shared` 根入口或 `@repo/shared/utils` 中的浏览器工具。
...
- **两套认证边界不得混用**：admin-api 后续采用管理端 JWT 与五表 RBAC；api 后续实现 C 端用户 JWT。当前都尚未落地，不得提前共享 Guard、会话或业务模块。
